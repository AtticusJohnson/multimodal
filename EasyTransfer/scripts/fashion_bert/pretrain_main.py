...
class ImageBertPretrain(base_model):
    ...
    def build_logits(self, features, mode=None):
        ...
        mlm_logits, nsp_logits, mpm_logits, target_raw_patch_features, pooled_output, text_sequence_output, image_sequence_output = \
        ...
        logits = (mlm_logits, nsp_logits, mpm_logits, pooled_output, text_sequence_output, image_sequence_output)
        labels = (masked_lm_ids, masked_lm_weights, nx_sent_labels, target_raw_patch_features)

        return logits, labels

    def build_loss(self, logits, labels):
        mlm_logits, nsp_logits, mpm_logits, pooled_output, text_sequence_output, image_sequence_output = logits
    ...
    def build_eval_metrics(self, logits, labels):
        mlm_logits, nsp_logits, _, pooled_output, text_sequence_output, image_sequence_output = logits
...
def main():
    # os.system("ls -d $PWD/fashionbert_fashiongen_patch_train_1K > train_list.list_csv")
    # os.system("ls -d $PWD/fashionbert_fashiongen_patch_train_1K > dev_list.list_csv")
    ...
    if FLAGS.mode == "train_and_evaluate":
        ...
        export_reader = BundleCSVReader(input_glob=app.predict_input_fp,
                                      input_schema=app.input_schema,
                                      is_training=False,
                                      batch_size=app.predict_batch_size,
                                      worker_hosts=app.config.worker_hosts,
