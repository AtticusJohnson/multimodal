...
class PatchFeatureExtractor(PredictorInterface):
  ...
  def predict(self, input_data, batch_size, num_patches=(8, 8)):
    """
    using session run predict a number of samples using batch_size
    Args:
      input_data:  a list of numpy array, each array is a sample to be predicted
    ...
    for image in input_data:
      image = cv2.resize(image, (256 * num_patches[0], 256 * num_patches[1]))
...
def read_h5_file(file_path, BATCH_SIZE = 32, debug=False):
  # open the file
