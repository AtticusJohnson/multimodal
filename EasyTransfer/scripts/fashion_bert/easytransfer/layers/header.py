...
class MLMHead(Layer):
    ...
    def call(self, hidden_states, masked_lm_positions):
        hidden_states = gather_indexes(hidden_states, masked_lm_positions)  # [1, input_sequence_length=64, 12head&64dim=768]
        # print(hidden_states.shape)
        word_embeddings = self.embeddings.word_embeddings  # [30522, 768]
        # print(word_embeddings.shape) 
        hidden_states = self.dense(hidden_states)  # [64, 768]
        # print(hidden_states.shape) 
        hidden_states = self.LayerNorm(hidden_states, name="transform/LayerNorm")
        # print(hidden_states.shape)  # [64, 768]
        # 句子中64个词和词典里所有词的相似度
        logits = tf.matmul(hidden_states, word_embeddings, transpose_b=True)  # [64, 30522] 
        # print(logits.shape)
