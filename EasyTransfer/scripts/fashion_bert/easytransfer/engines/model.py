...
from scripts.tensor_hook import EmbeddingHook 
import copy
import json
import random
import numpy as np
...
class EzTransEstimator(object):
    def __init__(self, **kwargs):
        ...
        if self.config.mode == 'train' or self.config.mode == "train_and_evaluate" or \
            ...
            # checkpoint_path = "fashionbert_pretrain_model_fin/model.ckpt-54198"
    ...
    def _build_model_fn(self):
        def model_fn(features, labels, mode, params):
            if mode == tf.estimator.ModeKeys.TRAIN:
                ...
                # pooled_output: 'image_bert_pre_trained_model/bert/pooler/dense/Tanh_1:0'
                # text_sequence_output: 'image_bert_pre_trained_model/strided_slice_5:0'
                # image_sequence_output: 'image_bert_pre_trained_model/strided_slice_6:0'
                
                # _str_pooled_output = tf.strings.format("{}\n", pooled_output)
                # write_file_op = tf.map_fn(lambda x: tf.strings.format("{}\n", tf.squeeze(x)), pooled_output, dtype=tf.string)
                # 
                # write_file_op = tf.io.write_file("./str_pooled_output.txt", _str_pooled_output)
                # summary_hook = tf.train.SummarySaverHook(output_dir="./amazon_output", summary_op=write_file_op, save_steps=1)
                # hook_writer = tf.estimator.FinalOpsHook(
                #     write_file_op, final_ops_feed_dict=None
                # )self, 
                write_hook = EmbeddingHook({"text_sequence_output": 'image_bert_pre_trained_model/bert/pooler/dense/Tanh_1:0'})
                
                return tf.estimator.EstimatorSpec(mode=mode, 
                                                  loss=eval_loss,
                                                  evaluation_hooks=[write_hook])
    ...
    def run_train_and_evaluate(self, train_reader, eval_reader):
        ...
        tf.estimator.train_and_evaluate(self.estimator,
                                        ...
                                        eval_spec=eval_spec)
        # input_fn=train_reader.get_input_fn()
        # pred_fn = self.estimator.predict(input_fn)  
        # for pred_out in pred_fn:
        #     print()
    ...
    def run_predict(self, reader, writer=None, checkpoint_path=None, yield_single_examples=False):
        ...
        return self.estimator.evaluate(input_fn=reader.get_input_fn(), checkpoint_path=checkpoint_path)
        # for batch_idx, outputs in enumerate(self.estimator.evaluate(input_fn=reader.get_input_fn(),
        #                                                            checkpoint_path=checkpoint_path)):
